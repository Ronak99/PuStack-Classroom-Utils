...
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.AnimationMode;
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.CameraBound;
...
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.Point;
...
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.WhiteObject;
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.WhiteScenePathType;
import com.pustack.live_session_whiteboard.whiteboardSdk.internal.Logger;

/**
 * `Displayer` 类。该类为 {@link Room Room} 类和 {@link Player Player} 类的父类。
...
public class Displayer {
     ...
     * @since 2.11.4
     *
     * @param object key-value 格式的信息，必须为 {@link WhiteObject WhiteObject} 的子类。
     *
     */
     ...
     * @param promise `Promise<WhiteScenePathType>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `getScenePathType` 方法的调用结果：
     *                - 如果方法调用成功，将返回场景路径类型。详见 {@link WhiteScenePathType WhiteScenePathType}。
     *                - 如果方法调用失败，将返回错误信息。
     */
     ...
     * 获取当前房间内所有场景的信息。
     *
     * @param promise `Promise<Map<String, Scene[]>>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `getEntireScenes` 方法的调用结果：
     ...
     * @since 2.4.28
     *
     * @param mode 视角调整时的动画模式，详见 {@link AnimationMode AnimationMode}。
     */
     ...
     * @param eventListener 自定义事件回调，详见 {@link EventListener EventListener}。如果添加多个同名的事件回调，则之前添加的回调会被覆盖。
     *
     */
     ...
     *
     * @param eventName     想要监听的自定义事件名称。
     * @param eventListener 自定义事件回调，详见 {@link FrequencyEventListener FrequencyEventListener}。如果添加多个同名的事件回调，则之前添加的回调会被覆盖。
     ...
     * @param promise `Promise<Point>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `convertToPointInWorld` 的调用结果：
     *                - 如果方法调用成功，将返回点在世界坐标系上的坐标。详见 {@link Point Point}。
     *                - 如果方法调用失败，将返回错误信息。
     */
     ...
     * @since 2.5.0
     *
     * @param bound 视角边界，详见 {@link CameraBound CameraBound}。
     */
     ...
     * @param promise   `Promise<Bitmap>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `getScenePreviewImage` 方法的调用结果：
     ...
     *
     * @param scenePath 场景路径。
     * @param promise   `Promise<Bitmap>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `getSceneSnapshotImage` 方法的调用结果：
     *                  - 如果方法调用成功，将返回指定场景的截图。
     *                  - 如果方法调用失败，将返回错误信息。
     */
     ...
     *
     * @param camera 视角的参数配置，详见 {@link CameraConfig CameraConfig}。
     */
     ...
     * @since 2.2.0
     *
     * @param rectangle 视觉矩形的参数设置，详见 {@link RectangleConfig RectangleConfig}。
     */
