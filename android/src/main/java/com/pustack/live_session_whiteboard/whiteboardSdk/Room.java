...
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.AddPageParam;
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.AkkoEvent;
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.Appliance;
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.BroadcastState;
...
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.EventListener;
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.FrequencyEventListener;
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.GlobalState;
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.ImageInformation;
...
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.RoomMember;
...
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.Scene;
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.SceneState;
...
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.WhiteDisplayerState;
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.WindowAppParam;
import com.pustack.live_session_whiteboard.whiteboardSdk.internal.Logger;
import com.pustack.live_session_whiteboard.whiteboardSdk.internal.RoomDelegate;
...
public class Room extends Displayer {
     ...
     *
     * @param promise `Promise<Object>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `disconnect` 的调用结果：
     *                - 如果方法调用成功，则返回房间的全局状态。
     *                - 如果方法调用失败，则返回错误信息。
     */
     ...
     * @note
     * - 该方法为同步调用。
     * - 对于通过 {@link WhiteDisplayerState#setCustomGlobalStateClass(Class<T> classOfT) setCustomGlobalStateClass}
     ...
     * @note
     * - 该方法为异步调用。
     * - 对于通过 {@link WhiteDisplayerState#setCustomGlobalStateClass(Class<T> classOfT) setCustomGlobalStateClass} 方法设置的自定义 `GlobalState`，在获取后，可以直接进行强转。
     *
     * @param promise `Promise<GlobalState>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `getGlobalState` 的调用结果：
     ...
     *
     * @return 当前的白板工具状态，详见 {@link MemberState MemberState}。
     *
     */
     ...
     * @note 该方法为异步调用。
     *
     * @param promise `Promise<MemberState>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `getMemberState` 的调用结果：
     ...
     * - 房间的用户列表仅包含互动模式（具有读写权限）的用户，不包含订阅模式（只读权限）的用户。
     *
     * @param promise `Promise<RoomMember[]>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `getRoomMembers` 的调用结果：
     ...
     *
     * @param promise `Promise<BroadcastState>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `getBroadcastState` 的调用结果：
     ...
     *   - {@link #putScenes(String, Scene[], int)}
     *
     * @param promise `Promise<SceneState>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `getSceneState` 的调用结果：
     ...
     *   - {@link #putScenes(String, Scene[], int) putScenes}
     *
     * @param promise `Promise<Scene[]>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `getScenes` 的调用结果：
     ...
     *
     * @param promise `Promise<Number>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `getZoomScale` 的调用结果：
     *                - 如果方法调用成功，则返回视角缩放比例。
     *                - 如果方法调用失败，则返回错误信息。
     */
     ...
     * 此时，你可以调用 {@link #getRoomPhase(Promise < RoomPhase > promise) getRoomPhase}[2/2] 立即获取最新的房间连接状态。
     *
     * @param promise `Promise<RoomPhase>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `getRoomPhase` 的调用结果：
     ...
     *
     * @param promise `Promise<RoomState>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `getRoomState` 的调用结果：
     ...
     * @param promise `Promise<Boolean>` 接口，详见 {@link Promise Promise}。你可以通过该接口获取 `setScenePath` 的调用结果：
     *                - 如果方法调用成功，则返回 `true`.
     *                - 如果方法调用失败，则返回错误信息。
     */
     ...
     * @param promise `Promise<Boolean>` 接口，详见 {@link Promise Promise}。你可以通过该接口获取 `setSceneIndex` 的调用结果：
     *                - 如果方法调用成功，则返回 `true`。
     *                - 如果方法调用失败，则返回错误信息。
     */
     ...
     * @since 2.6.2
     *
     * @param promise `Promise<JSONObject>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `debugInfo` 的调用结果：
     *                - 如果方法调用成功，则返回调试日志信息。
     *                - 如果方法调用失败，则返回错误信息。
     */
     ...
     *                 - `true`：互动模式，即具有读写权限。
     *                 - `false`：订阅模式，即具有只读权限。
     * @param promise  `Promise<Boolean>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `setWritable` 的调用结果：
     ...
     *
     * @param eventEntry 自定义事件内容，详见 {@link AkkoEvent AkkoEvent}。
     */
