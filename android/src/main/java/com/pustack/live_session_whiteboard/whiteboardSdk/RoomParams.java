...
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.Region;
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.WhiteObject;
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.WindowParams;
...
public class RoomParams extends WhiteObject {
     ...
     *
     * @note
     * 为确保传入的 `userPayload` 格式正确，必须使用 {@link WhiteObject WhiteObject} 子类。
     *
     ...
     * @param roomToken   用于鉴权的 Room Token。
     * @param uid 用户标识，可以为任意 string，字符串长度不能超过 1024, 2.15.0 后必须填写。
     * @param userPayload 自定义用户信息，必须为 {@link WhiteObject WhiteObject} 子类。
     */
     ...
     *
     * @return 自定义用户信息，详见 {@link MemberInformation MemberInformation}。
     */
    @Deprecated
