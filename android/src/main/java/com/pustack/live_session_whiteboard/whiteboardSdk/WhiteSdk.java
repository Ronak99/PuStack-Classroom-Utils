...
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.FontFace;
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.PlayerConfiguration;
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.PlayerTimeInfo;
...
import com.pustack.live_session_whiteboard.whiteboardSdk.domain.UrlInterrupter;
import com.pustack.live_session_whiteboard.whiteboardSdk.internal.PlayerJsInterfaceImpl;
import com.pustack.live_session_whiteboard.whiteboardSdk.internal.RoomJsInterfaceImpl;
import com.pustack.live_session_whiteboard.whiteboardSdk.internal.RtcJsInterfaceImpl;
import com.pustack.live_session_whiteboard.whiteboardSdk.internal.SdkJsInterfaceImpl;
import com.pustack.live_session_whiteboard.whiteboardSdk.internal.WsJsInterfaceImpl;
...
public class WhiteSdk {
    ...
    private RoomJsInterfaceImpl roomJsInterface;
    private PlayerJsInterfaceImpl playerJsInterface;
    private SdkJsInterfaceImpl sdkJsInterface;
     ...
     * @param context               安卓活动 (Android Activity) 的上下文。
     * @param whiteSdkConfiguration SDK 实例的配置，详见 {@link WhiteSdkConfiguration WhiteSdkConfiguration}。
     * @param urlInterrupter        图片 URL 拦截设置，详见 {@link UrlInterrupter UrlInterrupter}。@deprecated 该参数已废弃。请改用 `CommonCallbacks` 接口中的 {@link CommonCallbacks#urlInterrupter(String) urlInterrupter} 方法。
     */
     ...
     * @param roomPromise `Promise<Room>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `joinRoom` 的调用结果：
     ...
     * @param playerPromise       `Promise<Player>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `createPlayer` 的调用结果：
     * - 如果方法调用成功，将返回新创建的回放房间对象，详见 {@link Player Player}。
     * - 如果方法调用失败，将返回错误信息。
     */
     ...
     *
     * @param playerConfiguration 白板回放的参数配置，详见 {@link PlayerConfiguration PlayerConfiguration}。
     * @param listener            白板回放事件的回调，详见 {@link PlayerEventListener PlayerEventListener}。
     * @param playerPromise       `Promise<Player>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `createPlayer` 的调用结果：
     *  - 如果方法调用成功，将返回新创建的回放房间对象，详见 {@link Player Player}。
     *  - 如果方法调用失败，将返回错误信息。
     */
     ...
     *
     * @param playerConfiguration 白板回放的参数配置，详见 {@link PlayerConfiguration PlayerConfiguration}。
     * @param playablePromise     `Promise<Boolean>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `isPlayable` 的调用结果：
     *                            - 如果方法调用成功，则返回 `true`。
     *                            - 如果方法调用失败，则返回 `false`。
     */
     ...
     *
     * @param fontFaces   指定的字体，详见 {@link FontFace FontFace}。
     * @param loadPromise `Promise<JSONObject>` 接口实例，详见 {@link Promise Promise}。你可以通过该接口获取 `loadFontFaces` 的调用结果：
